...
### Feature error-surface audit

Every feature checkpoint must include a local error-surface audit before the checkpoint can be reported complete. The audit is about runtime/test errors, not visual design:

- For any browser run that is actually in scope, collect `console.error`, uncaught page exceptions, unhandled promise rejections, and failed browser requests using the test runner’s event hooks. Playwright implementations should attach listeners such as `page.on('console')`, `page.on('pageerror')`, and `page.on('requestfailed')` before exercising the feature.
- For CLI, backend, protocol, worker, database, and native work, inspect command stderr, test-runner failures, process exit status, server/worker logs, panic/crash output, and unhandled-exception reports.
- Treat warnings as recorded observations unless the project marks them as release-blocking. Treat errors and unexpected failed requests as `FAIL` or `BLOCKED` until reproduced, explained, fixed, or explicitly approved as expected.
- Expected errors require a narrow local allowlist and an assertion proving they are intentional; broad suppression such as “ignore all console errors” is prohibited.
- Do not put secrets, tokens, private payloads, or unnecessary personal data into logs or error artifacts. Keep captures local and task-scoped, and remove disposable artifacts during cleanup.
- Record the audit command or hook, evidence level, error count, warning count, allowlisted expectations, and unresolved issues in the checkpoint/handoff.

If no browser run was requested, do not launch Playwright merely for this audit. Mark browser console evidence `NOT_RUN` and complete the applicable local CLI/process-log audit instead. Console auditing is not a visual review and does not require screenshots.
...
- When a Playwright run is in scope, collect console errors, uncaught page errors, unhandled browser exceptions, and failed requests as part of the feature checkpoint; screenshots are not required for this console audit.
