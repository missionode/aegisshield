...
- Browser checks: headless Playwright when explicitly requested or when the task scope declares browser evidence; console/error-surface auditing is required whenever a browser run occurs.
...
Codex invokes Playwright only when the user explicitly requests browser testing or the task scope explicitly includes browser evidence. A browser interface alone is not permission to launch it. Tests run against the local target application and local supporting services with disposable local data.

Every feature checkpoint includes an error-surface audit. During a browser run, capture `console.error`, page errors, unhandled exceptions/rejections, and failed requests through the runner’s event hooks. For non-browser work, inspect stderr, test-runner output, process logs, and crash/unhandled-exception output. Unexpected errors block completion until resolved or explicitly approved as expected; warnings are reported separately. This audit does not require screenshots or manual visual review.
