...
Headless Playwright is the browser-level test standard for browser-facing applications when the user requests browser testing or the task scope explicitly includes browser evidence. Codex must not invoke it solely because a browser interface exists. It validates user-visible behavior, browser WebSocket behavior, authentication/session flows, subscriptions, progress, completion, reconnect/resume, error states, keyboard behavior, and responsive behavior. Responsive page-design checks use at least mobile `390x844`, tablet `768x1024`, and desktop `1440x900` local viewports unless the application declares more appropriate target dimensions. Manual visual review is optional and is not a release or completion gate.

Every feature checkpoint also performs an error-surface audit. During a browser run, capture console errors, uncaught page errors, unhandled promise rejections, and failed requests through Playwright or the selected browser runner. During CLI, protocol, backend, worker, or native validation, inspect stderr, test-runner output, process logs, crash output, and unhandled exceptions. Unexpected errors block completion; expected errors require narrow documented assertions. This audit does not require screenshots and does not authorize a browser run by itself.
