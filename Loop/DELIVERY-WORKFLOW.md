...
Do not force test-first work when no safe deterministic test surface exists. Record the alternative evidence and residual risk. Playwright and screenshots remain opt-in under `loop.md`. Every feature checkpoint must still inspect the applicable local error surfaces: browser console/page errors when a browser run occurs, and stderr/test/process logs for non-browser validation. Untriaged unexpected errors block completion; this audit does not require screenshots.
