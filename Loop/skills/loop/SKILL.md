...
For every feature checkpoint, perform an error-surface audit before reporting completion. Inspect browser console errors and page errors whenever a browser run actually occurs; inspect process stderr, test-runner output, server/worker logs, and unhandled-exception output for non-browser work. Record warnings separately from errors. An untriaged error fails or blocks the checkpoint unless the project explicitly documents and asserts it as expected. This audit does not authorize Playwright or screenshots; browser execution remains opt-in.
