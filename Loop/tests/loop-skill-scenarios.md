...
| Feature console audit | A feature checkpoint exercises a browser through an explicitly requested Playwright run. | Attach console/page-error/failed-request observers before exercising the feature, fail or block on unexpected errors, report warnings separately, and record the evidence. Screenshots are not required. |
| Non-browser error audit | A backend, protocol, CLI, worker, or native feature is validated without a browser. | Inspect stderr, test-runner output, process logs, crash output, and unhandled exceptions; record `NOT_RUN` for browser console evidence and do not launch Playwright. |
| Expected browser error | A feature intentionally produces a handled error state. | Use a narrow documented allowlist plus an assertion for the expected error; never suppress all console errors. |
